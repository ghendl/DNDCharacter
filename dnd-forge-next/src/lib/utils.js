...
export function summarize(form, refs) {
  ...
  const hpPerLevel = Number(form.hpPerLevel || Math.max(1, Math.floor(classDef.hitDie / 2) + 1));
  const hpBonus = Number(form.hpFlatBonus || 0);
  const hp = Number(form.hpOverride || 0) > 0
    ? Number(form.hpOverride)
    : classDef.hitDie + conMod + (level - 1) * Math.max(1, hpPerLevel + conMod) + hpBonus;
