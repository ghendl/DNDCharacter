...
function featSlotsFor(classDef, level = state.form.level) {
  if (!classDef?.progression) return [];
  const max = Number(level || 1);
  const slots = [];
  for (let lvl = 1; lvl <= max; lvl++) {
    const features = classDef.progression[lvl] || [];
    if (features.includes('Feat')) slots.push({ level: lvl, type: 'feat' });
    if (features.includes('Epic Boon')) slots.push({ level: lvl, type: 'epic' });
  }
  return slots;
}

function normalizeLevelChoices() {
  const slots = featSlotsFor(currentClass(), state.form.level);
  const existing = Array.isArray(state.form.levelChoices) ? state.form.levelChoices : [];
  state.form.levelChoices = slots.map((slot, index) => {
    const found = existing.find(item => Number(item.level) === slot.level && item.type === slot.type) || existing[index] || {};
    const mode = slot.type === 'epic' ? (found.mode || 'feat') : (found.mode || 'feat');
    return {
      level: slot.level,
      type: slot.type,
      mode,
      feat: found.feat || (slot.type === 'epic' ? 'Epic Boon' : ''),
      abilityA: found.abilityA || currentClass().primary?.[0] || 'str',
      abilityB: found.abilityB || currentClass().primary?.[1] || currentClass().primary?.[0] || 'con',
      note: found.note || '',
    };
  });
}

function levelChoiceBonuses() {
  const bonus = emptyAbilityMap();
  for (const choice of state.form.levelChoices || []) {
    if (choice.mode === 'asi2' && choice.abilityA) bonus[choice.abilityA] += 2;
    if (choice.mode === 'asi1_1') {
      if (choice.abilityA) bonus[choice.abilityA] += 1;
      if (choice.abilityB) bonus[choice.abilityB] += 1;
    }
  }
  return bonus;
}

function mergeAbilityBonuses(baseBonus, extraBonus) {
  const out = emptyAbilityMap();
  ABILITIES.forEach(a => { out[a.key] = Number(baseBonus[a.key] || 0) + Number(extraBonus[a.key] || 0); });
  return out;
}

function levelChoiceSummary(choice) {
  if (!choice) return '—';
  if (choice.mode === 'asi2') return `ASI +2 a ${ABILITY_LABELS[choice.abilityA]}`;
  if (choice.mode === 'asi1_1') return `ASI +1 a ${ABILITY_LABELS[choice.abilityA]} y ${ABILITY_LABELS[choice.abilityB]}`;
  return choice.feat || (choice.type === 'epic' ? 'Epic Boon pendiente' : 'Feat pendiente');
}

function subclassFeaturesUnlocked() {
  const subclassData = selectedSubclass();
  if (!subclassData?.features) return [];
  return subclassData.features.filter(feature => Number(feature.level) <= Number(state.form.level || 1));
}

function classChoicesSummary() {
  const cls = currentClass();
  const rows = classFeaturesUntil(cls, Number(state.form.level || 1));
  return rows.filter(row => row.features.some(feature => /Feat|Subclass Feature|Epic Boon/i.test(feature)));
}

function levelPlannerCard() {
  const slots = state.form.levelChoices || [];
  const cls = currentClass();
  const summaryRows = classChoicesSummary();
  return `
    <section class="card stack leveling-card">
      <div class="wizard-headline"><div><div class="eyebrow">Leveling</div><h3>Subidas de nivel y decisiones</h3></div><span class="status-chip info">${slots.length} slot(s)</span></div>
      <div class="grid-3 compact-kpi-grid">
        <div class="callout compact"><strong>HP por nivel</strong><p>d${cls.hitDie} · base ${Math.max(1, Math.floor(cls.hitDie / 2) + 1)}</p></div>
        <label>HP por nivel<input data-field="hpPerLevel" type="number" min="1" max="${cls.hitDie}" value="${state.form.hpPerLevel || Math.max(1, Math.floor(cls.hitDie / 2) + 1)}" /></label>
        <label>HP override<input data-field="hpOverride" type="number" min="0" placeholder="auto" value="${escapeHtml(state.form.hpOverride || '')}" /></label>
      </div>
      ${slots.length ? `<div class="level-choice-list">${slots.map((choice, idx) => `
        <div class="level-choice-card">
          <div class="row between center"><strong>Nivel ${choice.level}</strong><span class="status-chip ${choice.type === 'epic' ? 'warn' : 'ok'}">${choice.type === 'epic' ? 'Epic Boon' : 'Feat / ASI'}</span></div>
          <div class="grid-3">
            <label>Tipo
              <select data-level-choice="${idx}" data-level-prop="mode">
                <option value="feat" ${choice.mode === 'feat' ? 'selected' : ''}>Feat</option>
                <option value="asi2" ${choice.mode === 'asi2' ? 'selected' : ''}>ASI +2</option>
                <option value="asi1_1" ${choice.mode === 'asi1_1' ? 'selected' : ''}>ASI +1/+1</option>
              </select>
            </label>
            ${choice.mode === 'feat' ? `<label>Feat
              <select data-level-choice="${idx}" data-level-prop="feat">
                ${(choice.type === 'epic' ? ['Epic Boon', ...FEATS] : FEATS).map(feat => `<option value="${escapeHtml(feat)}" ${feat === choice.feat ? 'selected' : ''}>${escapeHtml(feat)}</option>`).join('')}
              </select>
            </label>` : `<label>Atributo A
              <select data-level-choice="${idx}" data-level-prop="abilityA">
                ${ABILITIES.map(a => `<option value="${a.key}" ${choice.abilityA === a.key ? 'selected' : ''}>${a.label}</option>`).join('')}
              </select>
            </label>`}
            ${choice.mode === 'asi1_1' ? `<label>Atributo B
              <select data-level-choice="${idx}" data-level-prop="abilityB">
                ${ABILITIES.map(a => `<option value="${a.key}" ${choice.abilityB === a.key ? 'selected' : ''}>${a.label}</option>`).join('')}
              </select>
            </label>` : `<label>Nota<input data-level-choice="${idx}" data-level-prop="note" value="${escapeHtml(choice.note || '')}" placeholder="Opcional" /></label>`}
          </div>
          <p class="muted">${escapeHtml(levelChoiceSummary(choice))}</p>
        </div>`).join('')}</div>` : '<div class="callout"><p>No hay feats o epic boons desbloqueados todavía para este nivel.</p></div>'}
      <div class="callout info">
        <strong>Resumen de decisiones desbloqueadas</strong>
        <p>${summaryRows.map(row => `Nivel ${row.level}: ${featureText(row.features)}`).join(' · ') || 'Todavía no hay elecciones especiales desbloqueadas.'}</p>
      </div>
      <label>Notas de leveling<textarea data-field="levelingNotes">${escapeHtml(state.form.levelingNotes || '')}</textarea></label>
      <label>Notas de magia / spellbook<textarea data-field="spellbookNotes">${escapeHtml(state.form.spellbookNotes || '')}</textarea></label>
    </section>`;
}
...
function recalcForm() {
  ...
  const originBonuses = computeBonuses(state.form.originPattern, priorities);
  normalizeLevelChoices();
  const progressionBonuses = levelChoiceBonuses();
  state.form.bonusAbilities = mergeAbilityBonuses(originBonuses, progressionBonuses);
  ...
  if (!state.form.hpPerLevel) state.form.hpPerLevel = Math.max(1, Math.floor(classDef.hitDie / 2) + 1);
      ...
      case 'identity':
        return `
          ...
          <section class="card stack wizard-step-card">
            ...
            <div class="callout compact"><strong>Resumen rápido</strong><p>Elegí base, campaña y nivel acá. La selección visual de species, clase y subclase sigue en el próximo paso para que mobile tenga más aire.</p></div>
            <label>Notas<textarea data-field="notes">${escapeHtml(state.form.notes)}</textarea></label>
          </section>`;
      case 'origin':
        return `
          <section class="card stack wizard-step-card">
            <div class="wizard-headline"><div><div class="eyebrow">Paso 2</div><h3>Origen, species, clase y feat</h3></div><span class="status-chip ok">Regla 2024</span></div>
            <div class="selection-stack compact-mobile-stack">
              <div>
                <div class="row between center"><strong class="picker-title">Species</strong><span class="status-chip info">${escapeHtml(species.name)}</span></div>
                <div class="icon-picks-grid horizontal-mobile-picks">
              ...
              <div>
                <div class="row between center"><strong class="picker-title">Clase</strong><span class="status-chip info">${escapeHtml(cls.name)}</span></div>
                <div class="icon-picks-grid compact horizontal-mobile-picks">
              ...
              <div>
                <div class="row between center"><strong class="picker-title">Subclase</strong><span class="status-chip ok">${cls.subclasses.length} opciones</span></div>
                <input id="subclass-search" placeholder="Buscar subclase" value="${escapeHtml(state.filters.subclassSearch)}" />
                <div class="subclass-grid horizontal-mobile-subclasses">
      ...
      default:
        return `
          <section class="card stack wizard-step-card">
            ...
            ${subclassData ? `<div class="callout info"><strong>${escapeHtml(state.form.subclass)}</strong><p>${escapeHtml(subclassData.summary || '')}</p><div class="feature-timeline">${subclassFeaturesUnlocked().map(feature => `<div class="feature-card"><span>Nivel ${feature.level}</span><strong>${escapeHtml(feature.name)}</strong><p>${escapeHtml(feature.desc)}</p></div>`).join('') || '<p class="muted">Rasgos de subclase futuros en la pestaña de progresión.</p>'}</div></div>` : ''}
            <div class="callout"><strong>Leveling</strong><p>${(state.form.levelChoices || []).map(choice => `Nivel ${choice.level}: ${levelChoiceSummary(choice)}`).join(' · ') || 'Todavía no tenés feats / ASI / epic boons desbloqueados.'}</p></div>
          </section>`;
    }
  }
...
function renderProgression() {
  ...
  const unlockedSubclassFeatures = subclassFeaturesUnlocked();
  return `
  <section class="section-grid progression-grid">
    ...
    <section class="card stack">
      ...
      <div class="callout info">
        ...
        ${unlockedSubclassFeatures.length ? `<div class="feature-timeline">${unlockedSubclassFeatures.map(feature => `<div class="feature-card"><span>Nivel ${feature.level}</span><strong>${escapeHtml(feature.name)}</strong><p>${escapeHtml(feature.desc)}</p></div>`).join('')}</div>` : '<p class="muted">Todavía no desbloqueaste rasgos de subclase a este nivel.</p>'}
      </div>
      <div class="callout">
...
function renderSummary() {
  ...
  <section class="section-grid">
    ...
    <section class="card stack">
      ...
      <div class="callout"><strong>Feats / ASI / Boons</strong><p>${(state.form.levelChoices || []).map(choice => `Nivel ${choice.level}: ${levelChoiceSummary(choice)}`).join(' · ') || 'Todavía no desbloqueaste decisiones de subida de nivel.'}</p></div>
      ${state.form.spellbookNotes ? `<div class="callout"><strong>Notas de magia</strong><p>${escapeHtml(state.form.spellbookNotes)}</p></div>` : ''}
      ${state.form.levelingNotes ? `<div class="callout"><strong>Notas de leveling</strong><p>${escapeHtml(state.form.levelingNotes)}</p></div>` : ''}
...
function bindEvents() {
  ...
  app.querySelectorAll('[data-level-choice]').forEach(el => {
    const commitChoice = () => {
      const idx = Number(el.dataset.levelChoice);
      const prop = el.dataset.levelProp;
      if (!state.form.levelChoices[idx]) return;
      state.form.levelChoices[idx][prop] = el.type === 'number' ? Number(el.value) : el.value;
      recalcForm();
      render();
    };
    el.addEventListener('change', commitChoice);
    if (el.tagName === 'INPUT') el.addEventListener('blur', commitChoice);
  });
